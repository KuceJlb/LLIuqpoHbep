...
class Source : public Expression
{
  ...
  {
    double x0 = atan((x[0]-0.5)/(x[1]-0.5)); 
    double x1 = pow((x[1]-0.5)*(x[1]-0.5)+(x[0]-0.5)*(x[0]-0.5), 0.5);
    double f = sin(4*x0)+2;
    if ((x1 < 0.2*f) and (x1 > 0.15*f)){//and (x1 > 4*f)
      values[0] = 10000;
    } else{
    values[0] = 0;
    }
...
public:
    ...
    void eval(Array<double>& values, const Array<double>& x) const override
    {
        double x0 = x[0];
        double x1 = x[1];
        values[0] = 0;//sin(pow(x0, 2) + pow(x1, 2));
    }
};
int main()
{
...
// .. code-block:: cpp
  ...
  auto u0 = std::make_shared<SinExpr>();
