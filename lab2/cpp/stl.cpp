...
public:
    ...
    CalcNode() : x(0.0), y(0.0), z(0.0), smth(0.0), vx(0.0), vy(0.0), vz(0.0), time(0.0)
    {
    }
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
	    ...
	    double vy = -pointX;
            ...
            double smth = sin(pow(pointX, 2) + pow(pointY, 2) + pow(pointZ, 2));
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, vx, vy, 3);
        }
